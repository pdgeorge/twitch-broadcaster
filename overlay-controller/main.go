...
func extractBadges(event map[string]any) []chatBadge {
	...
	for _, b := range badgesSlice {
		if badgeMap, ok := b.(map[string]any); ok {
			badge := chatBadge{
				SetID: firstString(badgeMap["set_id"], badgeMap["id"], ""),
				ID:    firstString(badgeMap["id"], ""),
			}
			badge.ImageURL = firstString(
				badgeMap["image_url_2x"],
				badgeMap["image_url_4x"],
				badgeMap["image_url_1x"],
			)
			if badge.SetID != "" || badge.ImageURL != "" {
...
func renderHTML(msg *chatMessage) string {
	...
	for _, b := range msg.Badges {
		...
		if b.ImageURL != "" {
			parts = append(parts, fmt.Sprintf("<img class='badge' src='%s' alt='%s' />", html.EscapeString(b.ImageURL), html.EscapeString(label)))
		} else if label != "" {
			parts = append(parts, fmt.Sprintf("<span class='badge'>[%s]</span>", html.EscapeString(label)))
		}
	}
