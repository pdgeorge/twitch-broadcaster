...
## Chat + Other overlay (OBS-friendly)

The overlay controller streams chat plus an "Other" box for announcements and a pong mini-game.

- Overlay assets live in `overlay/` (`index.html`, `overlay.css`, `overlay.js`). The CSS positions the chat box and the Other box so you can adjust placement without code changes.
...
- Other box controls:
  - `!other <markdown>` from broadcaster/mod sets the base Markdown content.
  - `!fire` from broadcaster/mod cancels an active announcement and restores the base.
  - Channel point redeem titled `announcement` overrides the Other box for 5 minutes with the redeem text (Markdown).
  - Pong triggers when any chat message contains "ping" or every 15 minutes automatically and runs for 60 seconds before returning to the base content.
